...
temp_dir = os.path.join(os.environ['TEMP'], 'hf_sync_fast')
...
print("Exporting tracked files via git archive...")
# git archive HEAD | tar -x -C %TEMP%\hf_sync_fast
res = subprocess.run(f"git archive HEAD | tar -x -C {temp_dir}", shell=True, capture_output=True, text=True)
if res.returncode != 0:
    print(f"Error archiving: {res.stderr}")
    exit(1)

print("Cleaning large files...")
for root, dirs, files in os.walk(temp_dir):
    for file in files:
        if file.endswith(('.csv', '.parquet', '.db', '.sqlite3', '.pkl', '.onnx', '.png', '.joblib', '.bin')) or file == 'kaggle_run_logs.txt':
