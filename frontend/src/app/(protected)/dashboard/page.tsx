...
export default function DashboardPage() {
  ...
  return (
    <div className="w-full min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)] font-sans selection:bg-[var(--accent)] selection:text-white pb-20">
      {/* Top Status Bar */}
      <div className="w-full bg-[var(--bg-secondary)] border-b border-[var(--border)] px-4 py-1.5 flex justify-between items-center text-[11px] font-mono tracking-widest text-[var(--text-dim)] uppercase" role="status" aria-label="System status bar">
        <div className="flex gap-4">
          <span className="flex items-center gap-1.5 text-[var(--success)]"><div className="w-1.5 h-1.5 bg-[var(--success)] rounded-full animate-pulse" aria-hidden="true" /> HL7 STREAM ACTIVE</span>
          <span>NODE: SECURE-MED-09</span>
      ...
      <div className="p-6 md:p-8 space-y-8 max-w-[1800px] mx-auto">
        ...
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-[var(--border)] pb-6">
          <div>
            <h1 className="text-3xl md:text-4xl font-semibold text-[var(--text-primary)] tracking-tight mb-1">
              Global Command Center
            </h1>
            <p className="text-[var(--text-secondary)] text-sm flex items-center gap-2">
              <BrainCircuit size={14} className="text-[var(--accent)]" />
              Singularity Medical Intelligence Engine v4.2.0 — Attending: {user?.full_name || user?.username || "Dr. Admin"}
            </p>
          </div>
          <div className="flex items-center gap-3">
            <Link href="/patients">
              <button className="btn btn-secondary text-xs font-semibold uppercase tracking-wider flex items-center gap-2" aria-label="Open EMR Database">
                <FileText size={14} /> EMR Database
              </button>
            </Link>
            <Link href="/chat">
              <button className="btn btn-primary text-xs font-semibold uppercase tracking-wider flex items-center gap-2" aria-label="Engage AI Copilot">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4" role="region" aria-label="Core telemetry metrics">
          {[
            ...
            <motion.div
              ...
              className="bg-[var(--bg-secondary)] border border-[var(--border)] p-5 flex flex-col justify-between relative overflow-hidden group"
            >
            ...
            <div className="flex-1 p-6 min-h-[350px]">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={displayChartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <XAxis dataKey="name" stroke="#404040" fontSize={10} tickLine={false} axisLine={false} dy={10} tick={{ fill: "#737373" }} />
                  <YAxis stroke="#404040" fontSize={10} tickLine={false} axisLine={false} dx={-10} tick={{ fill: "#737373" }} />
                  <Tooltip
                    cursor={{ stroke: "#404040", strokeWidth: 1, strokeDasharray: "4 4" }}
                    contentStyle={{ background: "#111111", border: "1px solid #262626", borderRadius: "4px", fontSize: "12px", fontFamily: "monospace" }}
                  ...
                  <Area
                    ...
                    fillOpacity={1}
                    fill="url(#colorRisk)"
                    activeDot={{ r: 4, fill: "#000000", stroke: "#0066cc", strokeWidth: 2 }}
                  />
                </AreaChart>
              </ResponsiveContainer>
            </div>
