...
### Milestone 2: Native Rust ONNX Runtime ML Inference Engine & Preprocessing Scalers
...
2. Update `rust_gateway/Cargo.toml` with `ort = { version = "2.0.0-rc.9", features = ["load-dynamic", "ndarray"] }` (or latest stable compatible `ort`), `ndarray = "0.16"`.
3. Implement `rust_gateway/src/ml/`:
   - `scalers.rs`: Native static const float arrays and $(X - \text{offset}) \times \text{scale}$ affine transformation functions for Kidney (24 features), Liver (10 features with `log1p` on indices `[2, 4, 5, 9]`), Lungs (15 features).
...
5. Add unit and integration tests in `rust_gateway/` verifying all models and scalers.
6. Execute standard verification: Explorer -> Worker -> Reviewer -> Challenger -> Auditor cycle.
7. Write `handoff.md` and send completion message to parent when done.
