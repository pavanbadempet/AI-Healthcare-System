...
### Milestone 4: Bun ElysiaJS API Orchestration Layer (BFF Gateway & Proxy)
**Working Directory**: c:\Users\pavan\OneDrive\Documents\GitHub\AI-Healthcare-System\.agents\sub_orch_m4_1
...
2. Create `edge_gateway/`:
   - `package.json`: Dependencies: `elysia`, `@elysiajs/cors`, `@elysiajs/jwt`, `@elysiajs/static` (or native static handler), `@elysiajs/websocket` / `@elysiajs/eden` if needed.
...
3. Test locally using `bun install` and `bun test` or verification scripts.
