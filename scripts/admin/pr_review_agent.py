#!/usr/bin/env python3
"""
Automated Pull Request Reviewer & Security Audit
================================================
Scans PR diffs for PII / secrets and performs automated code review checks.
"""
...
def review_pull_request(event_path: str, token: str):
    ...
    print(f"Auditing PR #{pr_number}: '{pr_title}'...")

    headers = {
        "Authorization": f"token {token}",
        "Accept": "application/vnd.github.v3+json",
        "User-Agent": "Automated-PR-Reviewer",
    }
    ...
    # Build Report
    report_lines = [f"### Automated Code Review for PR #{pr_number}\n"]
    report_lines.append(f"Audited **{audited_files} modified files** via **{ai_engine_name}**.\n")

    if issues_found:
        report_lines.append("#### Security Audit Warnings:")
    ...
    if ai_feedback:
        report_lines.append("#### Code Review Feedback:")
    ...
    report_lines.append("---")
    report_lines.append(f"*Generated by Automated PR Reviewer ({ai_engine_name}).*")
    ...
    if r_post.status_code in (200, 201):
        print(f"Posted review report on PR #{pr_number}")
    else:
