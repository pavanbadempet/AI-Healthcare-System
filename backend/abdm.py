...
# --- ABDM Schema and Connector Configurations ---
    ...
    *,
    ...
    if os.environ.get("MICROSERVICES_MODE") == "true":
        payload = {
            ...
            "status": status,
            "abdm_consent_id": abdm_consent_id,
            "hi_types": hi_types,
            "event_type": event_type,
            "notification_at": notification_at.isoformat() if notification_at else None,
            "error_code": error_code,
        }
        return _call_abdm_normalize_consent_callback(payload)
    import hashlib
...
def prepare_consent_request(
    ...
    if os.environ.get("MICROSERVICES_MODE") == "true":
        return _call_abdm_prepare_consent_request(
            patient_abha_address, purpose_code, hi_types, date_from, date_to, data_erase_at, None, hip_id, transport
        )
    settings = get_settings()
    ...
    # 2. Perform the actual HTTP POST request to the ABDM gateway endpoint
    headers = {
...
def verify_gateway_signature(signature_header: str | None, payload_bytes: bytes) -> bool:
    """
    Verifies that the callback request originated from the India ABDM Gateway.
    In production, this decodes the JWT using ABDM's JWKS public keys.
    In sandbox/demo mode, it validates the signature against the configured client secret or gateway token.
    """
    ...
    try:
        ...
        jwt.decode(
            token,
            key=settings.client_secret or "demo-secret",
            algorithms=["HS256", "RS256"],
            options={"verify_signature": not demo_mode, "verify_aud": False}
        )
        return True
    except Exception as e:
        logger.error("ABDM Gateway signature verification failed: %s", e)
        return False
