...
def _ensure_patient_slot_available(
    db: Session,
    patient_id: int,
    appointment_dt: datetime,
    *,
    ...
    if exclude_appointment_id is not None:
        query = query.filter(models.Appointment.id != exclude_appointment_id)
    if query.first():
        raise HTTPException(status_code=409, detail="Patient already has an active appointment at that time")
                    ...
                    if msg_type == "chunk":
                        ...
                        if booking_match:
                            ...
                            try:
                                ...
                                if existing_patient:
                                    raise ValueError("You already have an appointment booked at that time")
