...
# --- Microservices Mode Remote Call Fallbacks ---
...
# --- FHIR R4 Schema and Data Structures ---
...
def patient_resource(patient: Any) -> dict[str, Any]:
    if os.environ.get("MICROSERVICES_MODE") == "true":
        return _call_fhir_patient(patient)
...
def encounter_resource(encounter: Any, patient_id: int | str) -> dict[str, Any]:
    if os.environ.get("MICROSERVICES_MODE") == "true":
        return _call_fhir_encounter(encounter, patient_id)
    return _remove_none({
...
def observation_resource(observation: Any, patient_id: int | str) -> dict[str, Any]:
    if os.environ.get("MICROSERVICES_MODE") == "true":
        return _call_fhir_observation(observation, patient_id)
    components = []
...
def diagnostic_report_resource(result: Any, patient_id: int | str) -> dict[str, Any]:
    if os.environ.get("MICROSERVICES_MODE") == "true":
        return _call_fhir_diagnostic_report(result, patient_id)
    return _remove_none({
...
def medication_request_resource(prescription: Any, patient_id: int | str) -> dict[str, Any]:
    if os.environ.get("MICROSERVICES_MODE") == "true":
        return _call_fhir_medication_request(prescription, patient_id)
...
def care_event_resource(event: Any, patient_id: int | str) -> dict[str, Any]:
    if os.environ.get("MICROSERVICES_MODE") == "true":
        return _call_fhir_care_event(event, patient_id)
    return _remove_none({
...
def validate_fhir_resource(r: dict[str, Any]) -> None:
    ...
    if res_type == "Patient":
        if any(k not in ("resourceType", "id") for k in r.keys()):
            if "name" not in r or not isinstance(r["name"], list) or not r["name"]:
                raise FHIRValidationError("Patient resource must contain a non-empty name list")
        if "gender" in r:
        ...
        if "status" in r:
            if r["status"] not in ("registered", "preliminary", "final", "amended", "corrected"):
                raise FHIRValidationError("Observation must contain a valid status")
        if "code" in r:
            if not isinstance(r["code"], dict) or ("coding" not in r["code"] and "text" not in r["code"]):
                raise FHIRValidationError("Observation must contain a valid coding block")
        if "subject" in r:
            if not isinstance(r["subject"], dict) or "reference" not in r["subject"]:
                raise FHIRValidationError("Observation must contain a valid subject reference")
        ...
        if "status" in r:
            if r["status"] not in ("planned", "arrived", "triaged", "in-progress", "onleave", "finished", "cancelled", "open", "closed", "unknown"):
                raise FHIRValidationError("Encounter must contain a valid status")
        if "class" in r:
            if not isinstance(r["class"], dict) or "code" not in r["class"]:
                raise FHIRValidationError("Encounter must contain a valid class code")
        if "subject" in r:
            if not isinstance(r["subject"], dict) or "reference" not in r["subject"]:
                raise FHIRValidationError("Encounter must contain a valid subject reference")

    elif res_type == "MedicationRequest":
        if "status" in r:
            if r["status"] not in ("active", "on-hold", "cancelled", "completed", "entered-in-error", "stopped", "draft", "unknown"):
                raise FHIRValidationError("MedicationRequest must contain a valid status")
        if "intent" in r:
            if r["intent"] not in ("proposal", "plan", "order", "original-order", "reflex-order", "filler-order", "instance-order", "option"):
                raise FHIRValidationError("MedicationRequest must contain a valid intent")
        if "subject" in r:
            if not isinstance(r["subject"], dict) or "reference" not in r["subject"]:
                raise FHIRValidationError("MedicationRequest must contain a valid subject reference")

    elif res_type == "DiagnosticReport":
        if "status" in r:
            if r["status"] not in ("registered", "partial", "preliminary", "final", "amended", "corrected", "cancelled"):
                raise FHIRValidationError("DiagnosticReport must contain a valid status")
        if "code" in r:
            if not isinstance(r["code"], dict) or ("coding" not in r["code"] and "text" not in r["code"]):
                raise FHIRValidationError("DiagnosticReport must contain a valid coding block")
        if "subject" in r:
            if not isinstance(r["subject"], dict) or "reference" not in r["subject"]:
                raise FHIRValidationError("DiagnosticReport must contain a valid subject reference")

def build_bundle(resources: Iterable[dict[str, Any]], timestamp: Any = None) -> dict[str, Any]:
    if os.environ.get("MICROSERVICES_MODE") == "true":
        return _call_fhir_build_bundle(resources, timestamp)
    entries = []
    full_urls = set()
    present = set()
    for r in resources:
        validate_fhir_resource(r)
...
def audit_event_resource(audit_log: Any) -> dict[str, Any]:
    if os.environ.get("MICROSERVICES_MODE") == "true":
        return _call_fhir_audit_event(audit_log)
    return {
