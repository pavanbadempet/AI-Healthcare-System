...
def test_rust_engine_cosine_similarity():
    vec_a = [1.0, 2.0, 3.0]
    vec_b = [1.0, 2.0, 3.0]
    res = rust_bridge.compute_rust_cosine_similarity(vec_a, vec_b)
...
def test_rust_engine_egfr_calculation():
    ...
    egfr = rust_bridge.compute_rust_egfr(0.9, 45, is_female=True)
    assert egfr > 70.0
    assert egfr < 120.0
...
def test_rust_phi_redaction():
    ...
    redacted = rust_bridge.redact_phi_text_rust(text)
...
def test_rust_password_hashing():
    ...
    hashed = rust_bridge.hash_password_rust(pwd)
    assert hashed != pwd
    assert rust_bridge.verify_password_rust(pwd, hashed) is True
    assert rust_bridge.verify_password_rust("WrongPassword", hashed) is False

def test_rust_fedavg_aggregation():
    grads = [[1.0, 2.0], [3.0, 4.0]]
    weights = [0.5, 0.5]
    result = rust_bridge.aggregate_fedavg_rust(grads, weights)
